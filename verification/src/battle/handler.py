...
class FightHandler(BaseHandler):
    ...
    def start(self):
        ...
        yield self.compute_and_send_frame()
        # до и после вызовы связанны???
        self.create_map()
        self.create_route_graph()
        
        yield self.start_battle_members()
    ...
    @gen.coroutine
    def compute_and_send_frame(self):
        """
        ...
        if winner is not None:
            yield self.send_frame({'winner': winner}, True)
        else:
            IOLoop.current().call_later(self.FRAME_TIME, self.compute_and_send_frame)
    ...
    @gen.coroutine
    def start_battle_members(self):
        battle_members_coroutines = []
        for item in self.initial_data[INITIAL.MAP_ELEMENTS]:
            ...
            if item[ATTRIBUTE.ROLE] == ROLE.CRAFT:
                members = self.add_craft(item, player)
            else:
                members = (item,)

            for member in members:
                battle_members_coroutines.append(self.add_battle_member(member, player))
        yield battle_members_coroutines

    def add_craft(self, craft_data, player):
    ...
    @gen.coroutine
    def add_battle_member(self, item_data, player):
        ...
        # [SPIKE] We use center coordinates
        cut_size = max(size - self.CUT_FROM_BUILDING, 0)
    ...
    def send_frame(self, status=None, battle_finished=False):
        ...
        if self.is_stream:
            ...
            yield self.editor_client.send_battle({
                "is_stream": True,
                'status': status,
        ...
        if battle_finished:
            yield self.editor_client.send_battle(self.battle_log)

    @gen.coroutine
    def send_full_log(self):
        yield self.send_frame(battle_finished=True)
